...
const categories = [
{
id: “local”, icon: “🏛️”, title: “Local & Fornecedores”, color: “#c9a96e”,
items: [
...
{
id: “vestimenta”, icon: “👗”, title: “Vestimenta & Beleza”, color: “#d4a5c9”,
items: [
...
{
id: “convites”, icon: “✉️”, title: “Convites & Lista de Convidados”, color: “#9ec4c9”,
items: [
...
{/* Header */}
<div style={{ textAlign: “center”, padding: “48px 20px 18px”, position: “relative”, overflow: “hidden” }}>
<div style={{
position: “absolute”, top: 0, left: 0, right: 0, bottom: 0,
backgroundImage: “radial-gradient(ellipse at 50% 0%, rgba(201,169,110,0.15) 0%, transparent 70%)”,
pointerEvents: “none”,
}} />
<div style={{ fontSize: 12, letterSpacing: “0.45em”, color: “#c9a96e”, textTransform: “uppercase”, marginBottom: 10 }}>
Novembro 2026
</div>
<h1 style={{
...
<div style={{ color: “#c9a96e”, fontSize: 18, margin: “8px 0 20px”, letterSpacing: 8 }}>✦ ✦ ✦</div>
...
```
    ...
    <div style={{
      ...
      {syncPulse ? "Atualizado por outra pessoa ✓" : saving ? "Salvando..." : lastSaved ? `Salvo · compartilhado com todos` : "Checklist compartilhado"}
    </div>
  </div>
  ...
  {/* Filter */}
  <div style={{ display: "flex", justifyContent: "center", gap: 8, padding: "0 20px 22px", flexWrap: "wrap" }}>
    {[["all", "Todas"], ["pending", "Pendentes"], ["done", "Concluídas"]].map(([val, label]) => (
      <button key={val} onClick={() => setFilter(val)} style={{
        ...
        fontSize: 12, cursor: "pointer", letterSpacing: "0.06em", transition: "all 0.2s",
      }}>{label}</button>
    ))}
  </div>
      ...
      return (
        <div key={cat.id} style={{
          ...
          <button onClick={() => setOpenCat(isOpen ? null : cat.id)} style={{
            ...
            <span style={{ flex: 1, fontSize: 15, color: "#f0e6d3", letterSpacing: "0.02em" }}>{cat.title}</span>
            {allDone && <span style={{ fontSize: 10, color: "#7dc97a" }}>✓ COMPLETO</span>}
            <span style={{ fontSize: 12, color: allDone ? "#7dc97a" : cat.color, minWidth: 38, textAlign: "right" }}>
              {done}/{total}
            </span>
            <span style={{
              ...
              {visibleItems.map(item => {
                ...
                return (
                  <div key={item} onClick={() => toggle(cat.id, item)}
                    style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "8px 18px", cursor: "pointer" }}
  ...
  <div style={{ textAlign: "center", marginTop: 44, color: "#3a2a18", fontSize: 11, letterSpacing: "0.12em" }}>
    ✦ Feito com amor ✦
  </div>
</div>
```
